...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
  ...
  ret = -E_NO_MEM;
  //LAB5 2012011295 : (update LAB4 steps)
  //LAB4:EXERCISE2 2012011295
...
load_icode(int fd, int argc, char **kargv) {
  ...
  int stacktop = USTACKTOP - ROUNDUP(argv_size, sizeof(long));
  char** uargv = (char**)stacktop - argc * sizeof(char*);
  argv_size = 0;
  for (i = 0; i < argc; ++i) {
    uargv[i] = strcpy(stacktop + argv_size, kargv[i]);
    argv_size += strnlen(kargv[i], EXEC_MAX_ARG_LEN) + 1;
  }
  stacktop = uargv - sizeof(int);
  *(int*)stacktop = argc;
